...
package eu.artofcoding.flux.image

import eu.artofcoding.flux.helper.FileHelper
import eu.artofcoding.imagegallery.ConfigService
...
class ImageMagickService {
    ...
    public File convert(File image, File converted) {
        ...
        use(ImageMagickCategory) {
            // Convert
            Process process = image.init(configService.imageGalleryConfig.imageMagickHome).convert(converted)
    ...
    public File resize(File image, String width, String height, File converted = null) {
        ...
        if (!resized.exists()) {
            ...
            use(ImageMagickCategory) {
                // Convert
                Process process =
                    image.init(configService.imageGalleryConfig.imageMagickHome)
        ...
        if (!resized.exists()) {
            ...
            use(ImageMagickCategory) {
                ...
                if (decomposedImageFilename.ext != 'png') {
                    ...
                    image.init(configService.imageGalleryConfig.imageMagickHome).convert(pngFile)
                } else {
                    pngFile = image
                }
                // Create thumbnail
                ...
                try {
                    process = pngFile.init(configService.imageGalleryConfig.imageMagickHome)
                    ...
                    if (decomposedConvertedFilename.ext == 'gif') {
                        process = resized.init(configService.imageGalleryConfig.imageMagickHome)
                                .option('\\(')
                                .option('+clone')
    ...
    public Map identify(File image) {
        use(ImageMagickCategory) {
            image.init(configService.imageGalleryConfig.imageMagickHome).option('-verbose').identify()
        }
    }
